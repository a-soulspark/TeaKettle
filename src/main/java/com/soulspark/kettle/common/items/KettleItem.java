package com.soulspark.kettle.common.items;

import com.soulspark.kettle.Kettle;
import com.soulspark.kettle.common.blocks.KettleBlock;
	...
	public ActionResultType onItemUse(ItemUseContext context) {
		if (context.getPlayer() != null) {
			...
			if (result.getType().isSuccessOrConsume()) {
				Kettle.LOGGER.info(result.getResult().getTag());
...
//			Kettle.LOGGER.info("Hoopity dew! Take that: {} {}", rayTrace.getType(), rayTrace.getPos());
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
		...
		/*
		// TODO: don't forge to remove this debug code ;)
		if (!result.getType().isSuccessOrConsume()) {
		...
		if (getContentFromStack(stack).equals("empty")) {
			...
			if (rayTrace.getType() == RayTraceResult.Type.BLOCK) {
				...
				if (worldIn.getFluidState(pos).isTagged(FluidTags.WATER)) {
					...
					//if (!worldIn.isRemote) playerIn.setHeldItem(handIn, stack);
